...
const flowProg = mkP(
  ...
  `precision highp float;
   ...
   void main() {
     ...
     float bands[4];
     bands[0] = mix(baseLine, 2.2, uBass);
     bands[1] = mix(baseLine, 1.6, uLowMid);
     bands[2] = mix(baseLine, 1.1, uMid);
     bands[3] = mix(baseLine, 1.0, uHighMid);

     // Onset + bass swells all bands — kicks inhale the whole field
     float onsetBoost = 1.0 + uOnset * 1.6 + uBass * 0.5;
