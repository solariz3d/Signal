...
const prog2D = mkP(VS,
  `precision highp float;
   ...
   void main() {
     ...
     // Kick-flash field displacement — on kick, the noise field is pushed
     // radially outward near the pull center. The filaments respond to the
     // kick like liquid receiving an impact. This IS the flash, integrated
     // into the field physics instead of painted on top.
     vec2 flashDir = (st - pullCenter) / (pullRad + 0.001);
     float flashPush = pow(smoothstep(0.38, 0.0, pullRad), 1.4) * uSidechain * 0.22;
