...
function fingerprintTargetHues() {
  ...
  // All four hues are drawn from the palette (not triadic offsets from h1).
  // Each interpolates smoothly between adjacent palette entries as fingerprint
  // shifts, so colors stay continuous AND always land on curated jewel tones.
  if (arch.mode === 'curated') {
    ...
    // Four palette offsets — spaced so the three ridge hues are distinct jewel
    // tones and the 4th (pulse ride) is on the opposite side for contrast.
    const h1 = lerpP(i0, (i0 + 1) % p.length, f);
    const h2 = lerpP((i0 + 2) % p.length, (i0 + 3) % p.length, f);
    const h3 = lerpP((i0 + 5) % p.length, (i0 + 6) % p.length, f);
...
function render(now) {
  ...
  // Lower threshold (0.08) catches faster, subtler kicks in dense basslines.
