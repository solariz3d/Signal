...
const DPR = 1; // 1:1 pixel mapping for 1440p OLED
...
const flowProg = mkP(
  ...
  `precision highp float;
   ...
   uniform float uCameraZ, uCameraAngle;
   ...
   // Ridged FBM at a 3D point — the cosmic web density
   ...
   void main() {
     ...
     // === Camera setup ===
     // Camera flies forward along z, drifts laterally with angle
     vec3 camPos = vec3(
       sin(uCameraAngle) * 0.4,
       cos(uCameraAngle * 0.7) * 0.3,
       uCameraZ
     );
     // Look direction: forward along z with slight lateral wobble
     vec3 camTarget = camPos + vec3(
       sin(uCameraAngle * 1.3) * 0.1,
       cos(uCameraAngle * 0.9) * 0.08,
       1.0
     );
     ...
     // Ray direction for this pixel — FOV via the 0.8 multiplier
     vec3 rd = normalize(camFwd * 0.8 + camRight * st.x + camUp * st.y);

     // === Raymarching ===
     // Step through the noise field, accumulating glowing filament density
     vec3 col = vec3(0.0);
     ...
     for (int i = 0; i < 40; i++) {
       ...
       // Sample ridged noise at this point in space
       float churn = uChurnAmt * exp(-marchDist * 0.5); // churn fades with depth
       float density = ridgedFBM(p, t, uSeed, churn);

       // Skip empty space
       if (density < 0.01) continue;

       // Color varies with position — soul color mixing in 3D
       float m1 = snoise(p * 0.3 + vec3(1., 0., t * 0.15 + uSeed * 3.)) * 0.5 + 0.5;
       float m2 = snoise(p * 0.25 + vec3(-2., 0., t * 0.12 + uSeed * 5.)) * 0.5 + 0.5;
       vec3 soulHere = uSoulColor1 * m1 + uSoulColor2 * (1.0 - m1) * m2
                     + uSoulColor3 * (1.0 - m1) * (1.0 - m2);

       // Kick tints near-camera filaments with shifted hue
       ...
       // Depth fade — far filaments dimmer, near ones bright
       float depthFade = exp(-marchDist * 0.25);

       // Accumulate emission (front-to-back compositing)
       float alpha = density * stepSize * 3.0;
       alpha = min(alpha, 1.0 - totalDensity); // can't exceed 1
       col += filamentColor * density * alpha * depthFade * (1.5 + uRms * 0.5);
       totalDensity += alpha;
     ...
     // Void glow — subtle color in empty regions
...
const loc = {
  ...
  uChurnAmt: gl.getUniformLocation(flowProg, 'uChurnAmt'),
  uKickTransient: gl.getUniformLocation(flowProg, 'uKickTransient'),
  uCameraZ: gl.getUniformLocation(flowProg, 'uCameraZ'),
  uCameraAngle: gl.getUniformLocation(flowProg, 'uCameraAngle'),
...
let cameraZ = 0;    // accumulated forward position
let cameraAngle = 0; // lateral drift angle
...
function render(now) {
  ...
  // === Camera flight ===
  // Forward: base drift + bass cruise + kick boost
  const forwardSpeed = 0.2 + bassFloor * 0.8 + kickTransient * 3.0;
  cameraZ += dt * forwardSpeed;
  // Lateral drift: slow gentle orbit
  cameraAngle += dt * (0.06 + bassFloor * 0.1);
  ...
  gl.uniform1f(loc.uCameraZ, cameraZ);
  gl.uniform1f(loc.uCameraAngle, cameraAngle);
  ...
  if (diag) {
    ...
    diag.textContent = `[${arch}]  ${silent}  BASS ${b}  MID ${m}  HIGH ${h}  RMS ${r}  BPM ${bpm}  FPS ${Math.round(fpsEMA)}  (press C to cycle)`;
  }
}
