...
const flowProg = mkP(
  ...
  `precision highp float;
   ...
   uniform float uTime, uSeed, uZoom, uChurnAmt;
   ...
   void main() {
     ...
     // Wider smoothstep + no pow = softer, blurred edge on the center circle
     float centerWeight = smoothstep(0.50, 0.0, pullRad);
     ...
     float vig = smoothstep(1.1, 0.2, vigDist);
     col *= vig;
...
function render(now) {
  ...
  const churnTarget = frame.bass * 9.5 + frame.lowMid * 1.2 + gatedOnset * 7.0;
  const churnRate = churnTarget > churnAmt
    ? (1 - Math.exp(-dt * 25))   // attack ~40ms
    : (1 - Math.exp(-dt * 10));  // decay ~100ms — snappy pull-back
