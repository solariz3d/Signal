...
const prog2D = mkP(VS,
  `precision highp float;
   ...
   void main() {
     ...
     // Kick-flash field displacement — PULSAR jets. The noise field gets
     // pushed along the rotating jet axis (not radially), creating a
     // directional beam of displacement that sweeps around.
     ...
     float flashPush = pow(smoothstep(0.38, 0.0, pullRad), 1.4) * uSidechain * 0.28 * preJetAlign;
     vec3 st3_kick = st3 + vec3(flashDir * flashPush, 0.0) * uZoom;
     ...
     // --- Pulsar jets — spinning bidirectional beam concentrating pulse energy ---
     // The pulse isn't purely radially symmetric anymore. Two opposing jets
     // rotate slowly around the pull center; pulse brightness is amplified
     // along the jet axis. Reads as pulsar-like lighthouse sweep.
     float pulseAngle = atan(st.y - pullCenter.y, st.x - pullCenter.x);
     float jetAngle = uJetAngle;  // slow rotation (~18s per revolution)
     // abs(cos) gives bidirectional (two opposing jets); pow narrows the beam.
     float jetAlignment = pow(abs(cos(pulseAngle - jetAngle)), 4.0);
     // Jet gain: 1 away from jet axis, up to 3.5 along the jet on kick.
     float jetGain = 1.0 + jetAlignment * (1.5 + uSidechain * 4.0);
     ...
     // Jet gain concentrates energy along the rotating beam axis.
     col += kickColor * pulseShell * n * (1.6 + uBass * 1.3 + uSidechain * 7.0) * jetGain;
