...
const prog2D = mkP(VS,
  `precision highp float;
   ...
   void main() {
     ...
     // Mask spec contribution near the pull center so the natural streak
     // above the hole doesn't manifest.
     ...
     // Circular black hole — gated by bass, pulses with kick envelope.
     // uChurnAmt drives the pulse with the same attack/decay as the center
     // suck, so the hole expands on kicks and snaps back in sync.
     float holeBassGate = smoothstep(0.08, 0.2, uBass);
     // uHolePulse is positive on kick (grow), negative during rebound (shrink below baseline), back to 0 at rest
     float holeRadius = max(0.0, (0.07 + uBass * 0.03 + uHolePulse * 0.007) * holeBassGate);
     float blackHole = holeBassGate * (1.0 - smoothstep(holeRadius - 0.03, holeRadius + 0.05, pullRad));
...
const prog3D = mkP(VS,
  `precision highp float;
   ...
   void main() {
     ...
     // Three harmonics per axis with irrational multipliers (PHI, E) —
     // path covers more territory in varied directions without going faster.
     float PHI_S = 1.61803;
     float E_S = 2.71828;
     vec2 shipPos = vec2(
       sin(uTime * 0.073 * PHI_S) * 0.13 + cos(uTime * 0.041 * E_S) * 0.09 + sin(uTime * 0.023) * 0.06,
       cos(uTime * 0.055 * PHI_S) * 0.11 + sin(uTime * 0.038 * E_S) * 0.09 + cos(uTime * 0.029) * 0.07
     );
     ...
     for (int i = 0; i < 70; i++) {
       ...
       float pulseAmp = 1.0 / (1.0 + uRms * 2.0);
       vec3 displaced_p = p;
       float brightMod = 1.0;
       ...
       if (pulses.x < 5.0) {
         ...
         float w = shellWidth * shellWidth * (d1 > 0.0 ? 0.15 : 1.8);
         float shell = exp(-d1 * d1 / w) * exp(-pulses.x * 0.35);
       ...
       if (pulses.y < 5.0) {
         ...
         float w = shellWidth * shellWidth * (d2 > 0.0 ? 0.15 : 1.8);
         float shell = exp(-d2 * d2 / w) * exp(-pulses.y * 0.35);
         displaced_p += rd * shell * 0.12 * pulseAmp;
         brightMod += shell * 2.5 * pulseAmp;
         chromShift *= mix(vec3(1.0), vec3(1.5, 1.0, 0.45), shell * 0.6 * pulseAmp);
       }
       ...
       if (pulses.z < 5.0) {
         ...
         float w = shellWidth * shellWidth * (d3 > 0.0 ? 0.15 : 1.8);
         float shell = exp(-d3 * d3 / w) * exp(-pulses.z * 0.35);
         displaced_p += rd * shell * 0.12 * pulseAmp;
         brightMod += shell * 2.5 * pulseAmp;
         chromShift *= mix(vec3(1.0), vec3(1.5, 1.0, 0.45), shell * 0.6 * pulseAmp);
       }
       ...
       float density = ridgedFBM(displaced_p, t, uSeed, churn);
     ...
     // Plasma modulation — quiet bass = clean pinpoint, loud bass = roiling plasma.
     // Two octaves of noise animated at different rates churn across the core surface.
     ...
     // Mix strength scales with bass — silent ship = untextured, loud ship = plasma
     float plasmaMix = clamp(uBass * 1.4 + kickPulse * 0.6, 0.0, 1.0);
...
let pulseAges = [99, 99, 99];  // 3 active kick-pulses for wave propagation
...
function render(now) {
  ...
  // 3-slot pulse ring buffer — each kick shifts the array, oldest drops off
  if (kickTransient > 0.12) {
    pulseAges = [0, pulseAges[0], pulseAges[1]];
  }
  ...
  if (loc.uPulseAges) gl.uniform3f(loc.uPulseAges, pulseAges[0], pulseAges[1], pulseAges[2]);
