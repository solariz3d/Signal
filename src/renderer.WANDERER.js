  ...
  `precision highp float;
   ...
   void main() {
     ...
     for (int i = 0; i < 28; i++) {
       ...
       if (density < 0.06) continue;  // low threshold — structures always visible
       // Smoothstep shapes the filament — base structure visible, music brightens peaks
       density = smoothstep(0.06, 0.40, density) * 0.9;
       ...
       // Ship light illuminates nearby filaments — proximity to ship position
       float toShip = length(rd.xy - shipPos) * 2.0 + marchDist * 0.3;
       float shipLight = exp(-toShip * toShip * 0.6) * (0.5 + uBass * 1.5 + uKickTransient * 3.0);
       vec3 shipLightColor = mix(vec3(0.5, 0.7, 1.0), vec3(1.0, 0.6, 0.3), clamp(uKickTransient * 4.0, 0.0, 1.0));
       filamentColor += shipLightColor * shipLight * 0.6;

       float depthFade = exp(-marchDist * 0.25);

       float alpha = min(density * stepSize * 3.5, 1.0 - totalDensity);
     ...
     // Ship only exists when bass is present — gone completely otherwise
     float bassGate = smoothstep(0.05, 0.2, uBass);

     float coreGlow = exp(-shipDist * shipDist * 25.0);
     coreGlow *= (uBass * 1.6 + kickPulse * 2.0) * bassGate;

     float flareGlow = exp(-shipDist * shipDist * 4.0);
     flareGlow *= (uBass * 0.5 + kickPulse * 0.8) * bassGate;
     ...
     trail *= (uBass * 1.0) * bassGate;

     vec3 coreColor = mix(vec3(0.6, 0.8, 1.0), vec3(1.0, 0.7, 0.3), kickPulse);
...
btn2D.addEventListener('click', () => { currentMode = '2d'; loc = loc2D; btn2D.classList.add('active'); btn3D.classList.remove('active'); });
btn3D.addEventListener('click', () => { currentMode = '3d'; loc = loc3D; btn3D.classList.add('active'); btn2D.classList.remove('active'); });
