...
const prog2D = mkP(VS,
  `precision highp float;
   ...
   void main() {
     ...
     // Exposure tone-map — asymptotes cleanly to 1.0 even for very bright
     // near filaments during loud bass. Prevents the "full/bright" blow-out
     // while still letting quiet regions remain dim. Exposure 0.6 means a
     // raw value of 5 maps to ~0.95, and 15 maps to ~0.9998.
...
function fingerprintTargetHues() {
  ...
  // === EMERGENT — curated jewel-tone palette ===
  if (arch.mode === 'curated') {
    ...
    const h1 = wrap(p[i0] + d * f);
    const h2 = wrap(h1 + 0.333 + spread * 0.04);
    const h3 = wrap(h1 + 0.666 - spread * 0.04);
    // Pulse-ride hue — pull from opposite side of palette for contrast
    const i4 = (i0 + Math.floor(p.length / 2)) % p.length;
    const h4 = p[i4];
    return [h1, h2, h3, h4];
  }
...
function render(now) {
  ...
  if (loc.uPulseAges2) gl.uniform3f(loc.uPulseAges2, pulseAges[3], pulseAges[4], pulseAges[5]);
