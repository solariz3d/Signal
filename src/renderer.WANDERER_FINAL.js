  ...
  `precision highp float;
   ...
   void main() {
     ...
     for (int i = 0; i < 70; i++) {
       ...
       // Ship light — gated by bass so no afterglow when ship is off
       float shipBassGate = step(0.18, uBass);
       float toShip = length(rd.xy - shipPos) * 2.0 + marchDist * 0.3;
       float shipLight = exp(-toShip * toShip * 0.6) * (uBass * 0.3 + uKickTransient * 0.4) * shipBassGate;
     ...
     float bassGate = step(0.18, uBass);
